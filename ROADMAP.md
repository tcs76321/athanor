...
| M3 Dialectical Loop v1 | 🚧 In progress — T1 ✅ ([plan](docs/m3-t1-plan.md)); T2 ✅; T3 ✅; T4 ✅; T5–T7 next. M3-T1 shipped the full §8 phase executor + EvaluationRecord schema + §19.3 deterministic guard in 4 commits (`c73a115`, `9e99e20`, `9cb4ccd`, `62ae865`); the E1/E2/E4/E5 dialectical-loop test suite landed with the close-out. M3-T2 shipped in 6 commits (`a4e2362`–`97aed91`): the M2-T4 pod sub-steps moved from `phaseSynthesize` to `phaseEvaluate` (ADR-0014), the per-archetype §19.1 rubric drives per-candidate acceptance checks, the new `/internal/v1/jobs/{id}/lint` route extends the tool envelope to a closed three-tool set, the §19.3 guard is a pure function (`DecideWinner`, `internal/engine/decide.go`), the per-phase wall-time budget emits a `context_deadline_exceeded` audit row, and `spikes/m3-t2-probe/` exercises 5 code-archetype goals against the rubric. M3-T3 shipped in 4 commits (`2fbae3c`–`a5da467`): the comparison prompt includes a "Previous-record summary" section, the supersede+accept transition is atomic via `SupersedeAndAccept` (§9.3 status flow gains the `accepted → superseded` edge), `parseComparisonVerdict` trims whitespace and reports unknown winners via a typed error (no more silent downgrades), and the `DecideWinner` table tests gain 6 new rows for ties and boundaries. M3-T4 shipped in 3 commits (`6956a1f`–`dbe3478`+this one): the reflection counter is a typed `system_state` row (no more `jobs.recovery_flag` hack), the budget is a config field (`execution.max_reflection_loops`), and `validateCross` rejects non-positive values. Architectural decisions in [`docs/adr/0011`](docs/adr/0011-external-api-host-allowlist.md)–[`0014`](docs/adr/0014-evaluation-phase-move.md). `make check` clean; Gates G1 + G2 re-proven. |
