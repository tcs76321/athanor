...
| M2 Container Spine | 🚧 In progress — T1 (rootless podman spike, [ADR-0007](docs/adr/0007-podman-lifecycle.md)), T2 (Job Pod manager in `internal/jobpod`, production client in `cmd/athanor/jobpod_client.go`, sweep on daemon boot), and T3 (per-job tokens + `/internal/v1/` API with bearer-token auth, [ADR-0008](docs/adr/0008-job-pod-tokens.md); Gate G2 added in `internal/gate/gate_g2_test.go`) done; T4–T6 unblocked |
