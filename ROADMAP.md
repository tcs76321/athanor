...
| M4 Airlock & Gateway | 🟡 In progress (T1 done; T2–T8 pending) — T1 ([plan-implicit: §21.3 path containment](ARCHITECTURE.md#213-file-airlock)) shipped the §21.3 path containment library at `internal/airlock/paths` (`5f829b7` + Gate G1 rule 5 prep `54d8d7c`): `Resolve` (path arithmetic), `Validate` (mode + structural), `OpenNoFollow` (`O_NOFOLLOW` via build-tag-gated wrappers — the only `internal/` files that may import `syscall`). Property test (`testing/quick`, 200 iters) over the adversarial path corpus; round-trip test; behavioral `O_NOFOLLOW` test (skips on unsupported GOOSes); setuid/setgid rows self-skip on hosts whose filesystem drops the bit on `chmod`. `make check` clean; Gate G1 re-proven. T2 (ingress pipeline), T3 (ClamAV / YARA / prompt-injection scan), T4 (egress pipeline), T5 (gateway), T6 (Reader Mode), T7 (gateway-backed tools), T8 (adversarial security suite) remain. Gate G4 closes after the adversarial suite is green and a research goal completes end-to-end using only allowlisted domains. |
