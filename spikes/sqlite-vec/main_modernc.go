...
func main() {
	...
	testDriver(ctx, "modernc", getExtensionPath())
}
...
func testFTS5(ctx context.Context, conn *sql.Conn, name string) error {
	...
	conn.ExecContext(ctx, `INSERT INTO fts_items(rowid, content, title) VALUES (1, 'AI agent architecture design', 'Design doc')`)
	conn.ExecContext(ctx, `INSERT INTO fts_items(rowid, content, title) VALUES (2, 'Vector search with sqlite-vec', 'Tech note')`)
