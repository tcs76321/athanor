...
| M2 Container Spine | 🚧 In progress — T1 (rootless podman spike, [ADR-0007](docs/adr/0007-podman-lifecycle.md)), T2 (Job Pod manager in `internal/jobpod`, production client in `cmd/athanor/jobpod_client.go`, sweep on daemon boot), T3 (per-job tokens + `/internal/v1/` API with bearer-token auth, [ADR-0008](docs/adr/0008-job-pod-tokens.md); Gate G2 added in `internal/gate/gate_g2_test.go`), T4 (internal API `execute_code` + `run_tests` with per-job tool envelope enforced server-side; engine sub-steps in `phaseSynthesize` for `code` archetype; [ADR-0009](docs/adr/0009-engine-pod-wiring.md)), and T5 (terminal-state cleanup + token-dir revocation on supervisor exit) done; T6 in progress ([ADR-0010](docs/adr/0010-m2-security-suite.md), [plan](docs/m2-t6-plan.md)) |
